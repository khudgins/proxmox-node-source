...
class TestFetchProxmoxNodes:
    ...
    def test_fetch_vms_only(self, mock_proxmox):
        ...
        assert len(result) == 2
        assert all(node['attributes']['proxmox_type'] == 'qemu' for node in result)
    ...
    def test_fetch_containers_only(self, mock_proxmox):
        ...
        assert len(result) == 1
        assert result[0]['attributes']['proxmox_type'] == 'lxc'
    ...
    def test_fetch_both_vms_and_containers(self, mock_proxmox):
        ...
        assert len(result) == 3
        vm_count = sum(1 for node in result if node['attributes']['proxmox_type'] == 'qemu')
        container_count = sum(1 for node in result if node['attributes']['proxmox_type'] == 'lxc')
        assert vm_count == 2
    ...
    def test_node_structure(self, mock_proxmox):
        ...
        for node in result:
            ...
            assert 'proxmox_node' in node['attributes']
            assert 'proxmox_vmid' in node['attributes']
            assert 'proxmox_type' in node['attributes']
            assert 'proxmox_status' in node['attributes']
    
    def test_running_vm_has_running_flag(self, mock_proxmox):
        """Test that running VMs have proxmox_running attribute."""
        result = proxmox_node_source.fetch_proxmox_nodes(
            proxmox=mock_proxmox,
            include_vms=True,
            include_containers=False
        ...
        running_vm = next((node for node in result if node['attributes']['proxmox_vmid'] == '100'), None)
        assert running_vm is not None
        assert running_vm['attributes'].get('proxmox_running') == 'true'
        
        # Find the stopped VM (vmid 101)
        stopped_vm = next((node for node in result if node['attributes']['proxmox_vmid'] == '101'), None)
        assert stopped_vm is not None
        assert 'proxmox_running' not in stopped_vm['attributes']
    ...
    def test_vm_without_ip_uses_fallback(self, mock_proxmox):
        ...
        for node in result:
            if node['attributes']['proxmox_type'] == 'qemu':
    ...
    def test_multiple_cluster_nodes(self, mock_proxmox):
        ...
        assert all(node['attributes']['proxmox_node'] in ['pve1', 'pve2'] for node in result)
